using System;
using System.Collections.Generic;

namespace WebApplication1.Model
{
    public partial class VKlgfsdxczpBcjcdx
    {
        public string Id { get; set; }
        public string Cjrid { get; set; }
        public DateTime? Cjsj { get; set; }
        public string Shrid { get; set; }
        public DateTime? Shsj { get; set; }
        public int? Shzt { get; set; }
        public string Shbtgsm { get; set; }
        public sbyte? Sfkdj { get; set; }
        public string Djrid { get; set; }
        public DateTime? Djsj { get; set; }
        public sbyte? Sfydj { get; set; }
        public string Shyc { get; set; }
        public DateTime? Tjsj { get; set; }
        public string Lyjd { get; set; }
        public DateTime? Jcsj { get; set; }
        public string Zpmc { get; set; }
        public string Zplj { get; set; }
        public string Rwid { get; set; }
        public string Tjrid { get; set; }
        public string Mc { get; set; }
        public string Tplysjid { get; set; }
        public string Whycm { get; set; }
        public string Fwms { get; set; }
        public string Dlxxzb { get; set; }
        public string Glzrdw { get; set; }
        public string Tpmc { get; set; }
        public string Tpsjl { get; set; }
        public string Pssb { get; set; }
        public string Tpgs { get; set; }
        public DateTime? Tpscsj { get; set; }
        public string Tpcjzrr { get; set; }
        public string Sjcjdbh { get; set; }
        public string Cjdzbxx { get; set; }
        public string Tpcjdw { get; set; }
        public string Tpsdz { get; set; }
        public string Tpsddw { get; set; }
        public DateTime? Tpsdsj { get; set; }
        public string Sjmj { get; set; }
        public string Fbfw { get; set; }
        public DateTime? Sjzqsj { get; set; }
        public string Bdlj { get; set; }
        public string Ydlj { get; set; }
        public string Xzdz { get; set; }
        public int? Xzcs { get; set; }
        public string Zscqgs { get; set; }
        public string Qt1 { get; set; }
        public string Qt2 { get; set; }
        public string Txrid { get; set; }
        public DateTime? Txsj { get; set; }
        public string Tplysjshrid { get; set; }
        public DateTime? Tplysjshsj { get; set; }
        public string Jcsjx { get; set; }
        public string Jcsjid { get; set; }
        public string Cjrname { get; set; }
    }
}
