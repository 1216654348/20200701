using System;
using System.Collections.Generic;

namespace WebApplication1.NJCQModel
{
    public partial class VTShhjYcqhhcqshhjndjcjl
    {
        public string Id { get; set; }
        public short? Zykcdsl { get; set; }
        public short? Fmyxfw { get; set; }
        public short? Yzwrgyqysl { get; set; }
        public string Lrmlsdycqrksl { get; set; }
        public string Dqycqrksl { get; set; }
        public string Ycqrkmd { get; set; }
        public double? Rkssxq { get; set; }
        public string Rkssxqname { get; set; }
        public string Lrmlsdhcqrk { get; set; }
        public string Dqhcqrksl { get; set; }
        public string Nyzz { get; set; }
        public string Sdcybl { get; set; }
        public string Fwyzz { get; set; }
        public string Gyzz { get; set; }
        public string Cjrid { get; set; }
        public DateTime? Cjsj { get; set; }
        public string Shrid { get; set; }
        public DateTime? Shsj { get; set; }
        public short? Shzt { get; set; }
        public string Djrid { get; set; }
        public DateTime? Djsj { get; set; }
        public sbyte? Sfydj { get; set; }
        public string Shyc { get; set; }
        public DateTime? Rksj { get; set; }
        public string Glycbtid { get; set; }
        public string Nd { get; set; }
        public string Shbtgsm { get; set; }
        public string Rwid { get; set; }
    }
}
