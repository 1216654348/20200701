using System;
using System.Collections.Generic;

namespace WebApplication1.NJCQModel
{
    public partial class TYgjcTbCopy1
    {
        public string Id { get; set; }
        public string GeoJson { get; set; }
        public string Name { get; set; }
        public int? Year1 { get; set; }
        public int? Year2 { get; set; }
        public string Tbname { get; set; }
        public string Tbdesc { get; set; }
        public DateTime? Rksj { get; set; }
        public string Sjwhycbm { get; set; }
        public double? Mj { get; set; }
        public string Tdfl { get; set; }
        public string Tdflbhh { get; set; }
        public string Pg { get; set; }
        public string UserId { get; set; }
        public int? Tbbh { get; set; }
        public int? Gid { get; set; }
        public string Bhqh { get; set; }
        public string Qdm { get; set; }
        public string Hdm { get; set; }
        public string Sdhc { get; set; }
        public string Xczp { get; set; }
        public string Xcsp { get; set; }
        public DateTime? Hcsj { get; set; }
        public double? Xzb { get; set; }
        public double? Yzb { get; set; }
        public string Xwcd { get; set; }
        public string Jcsj { get; set; }
        public string Bz { get; set; }
        public string Xhzh { get; set; }
        public string Sjxm { get; set; }
        public string Scwz { get; set; }
        public string Sdlx { get; set; }
        public string Sfxjsjxm { get; set; }
        public string Xmmc { get; set; }
        public string Jzyt { get; set; }
        public string Xmjszt { get; set; }
        public string Sfzdwwbmty { get; set; }
        public DateTime? Kgsj { get; set; }
        public DateTime? Jgsj { get; set; }
        public decimal? Jzmj { get; set; }
        public decimal? Gd { get; set; }
        public int? Cs { get; set; }
        public string Sffhglgd { get; set; }
        public string Ztgjsfbh { get; set; }
        public string Shzt { get; set; }
        public DateTime? Shsj { get; set; }
        public string Shrid { get; set; }
        public string Shbtgsm { get; set; }
        public string Shyc { get; set; }
    }
}
